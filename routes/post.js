...
router.get("/data",(req,res,next)=>{
    ...
    sequelize.query(`CALL Adansa.ra_vdetails_list('',"summary")`).then((comp_res)=>{
            if(fromdate != null && todate!=null && comp!=null){
                ...
                sequelize.query("SELECT voucher.*,voucher.id AS vid,vdetail.cr AS CR_NOR, vdetail.dr AS DR_NOR, company_master.name,if(voucher.isFx=1,vitem.fxamt,if(voucher.isFx=0,vitem.amount,0)) AS invAmnt,if(voucher.isFx=1,if(vdetail.cr!=0,vdetail.fxamt,0),vdetail.cr) AS partyCr,if(voucher.isFx=1,if(vdetail.dr!=0,vdetail.fxamt,0),vdetail.dr) AS partyDr,ledgermaster.ledger_name AS partyName ,if(voucher.isFx=1,currency.symbol,voucher.currencySymbol) as currency FROM voucher INNER JOIN `company_master` ON voucher.cid=company_master.rlb_cid INNER JOIN `vdetail` ON (voucher.id = vdetail.vid AND vdetail.narration='r2@0' AND vdetail.status=1) INNER JOIN `ledgermaster` ON (voucher.id = vdetail.vid AND vdetail.narration='r2@0' AND vdetail.ledger=ledgermaster.id) INNER JOIN `vitem` ON (voucher.id = vitem.vid) left join `currency` ON (vdetail.currency_id=currency.id and vdetail.vid=voucher.id) WHERE voucher.status=1").then((result1)=>{
