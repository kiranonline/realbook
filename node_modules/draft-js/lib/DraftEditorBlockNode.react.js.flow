  ...
  render(): React.Node {
    ...
    if (block.children.size) {
      children = block.children.reduce((acc, key) => {
        ...
        const { Element, wrapperTemplate } = getDraftRenderConfig(child, blockRenderMap);
        const elementProps = getElementPropsConfig(child, editorKey, offsetKey, blockStyleFn, customConfig);
        const childProps = {
          ...this.props,
    ...
    const blockNode = <DraftEditorNode block={block} children={children} contentState={contentState} customStyleFn={customStyleFn} customStyleMap={customStyleMap} decorator={decorator} direction={direction} forceSelection={forceSelection} hasSelection={isBlockOnSelectionEdge(selection, blockKey)} selection={selection} tree={tree} />;
    ...
    const elementProps = getElementPropsConfig(block, editorKey, offsetKey, blockStyleFn, getCustomRenderConfig(block, blockRendererFn));
